...
def prepare_and_render(
    ...
    p_synthesis_directory (str): Directory to save synthesized audio files.
     (bool): Whether to enable extraction of audio from the
    ...
    #     print('rubberband and spleeter are not installed!')
    #     return
    ...
    # whisper模型调用太慢，如果指定路径存在已有文件就不需要重复处理
    if os.path.exists(join(download_sub_directory, "words.txt")):
        print('已执行过transcript和提取words，直接从文件加载...')
        from processing import load_words, load_transcription_data
        words = load_words(join(download_sub_directory, "words.txt"))
        transcript_root = join(download_sub_directory, "\htdemucs_ft\2_Preferences_and_Utility_Functions_tCKk22kaZi4")
    ...
    # if os.path.exists( "final_cut_audio_merged.wav"):
    #     print('已执行过diarization并生成final_cut_audio_merged.wav文件，直接从文件加载...\n 警告：建议后续将合并的audio放到下载目录')
    # else:
