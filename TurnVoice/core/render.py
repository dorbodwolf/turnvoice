...
def render_video(
    ...
    final_cut_audio_path = join(p_synthesis_directory, "final_cut_audio.wav")
    print(f"[{(time.time() - t_start):.1f}s] "
          "combining audio..."
          )
    if exists(final_cut_audio_path):
        print(f"已找到{final_cut_audio_path}文件，跳过合成")
    else:
    ...
    if not p_clean_audio:
        ...
        #
        final_cut_audio_merged = join(p_synthesis_directory, "final_cut_audio_merged.wav")
        ...
        if exists(final_cut_audio_merged):
            print(f"已找到{final_cut_audio_merged}文件，跳过合并")
        else:
